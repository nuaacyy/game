...
class EntityFinder(
    ...
    fun findAoeBySelf(
        ...
        entity.manager.entityList.forEach {
            ...
            //目标阵营筛选
            if (unitType == 1 && it.entityType != Hero) {
                return@forEach
            }
            if (unitType == 2 && it.entityType != Summoned) {
                return@forEach
            }
    ...
    fun findSceneBySelfPos(
        ...
        val launchingSkill = entity.launchingSkill
        if (launchingSkill != null) {
            sceneEntity.intPropertyMap[ProtoId] = launchingSkill.skillProto.id
        }
...
//过滤阵营
fun campFilter(entity: Entity, targetEntity: Entity, camp: Int): Boolean {
    //过滤阵营
    when (camp) {
        FriendlyForce -> {
            if (targetEntity.getIntProperty(FightSide, false) != entity.getIntProperty(FightSide, false)) {
                return false
            }
        }
        ...
        SelfForce -> {
            if (entity.id != targetEntity.id && targetEntity.intPropertyMap[AttachHeroId] != entity.id) {
                return false
            }
        }
    }
...
fun checkCamp(entity: Entity, camp: Int): Int {
    ...
    for ((_, buffList) in entity.buffMap) {
        for (buff in buffList) {
            ...
            for (handle in handlerList) {
                if (!handle(buff)) {
                    return FriendlyForceExceptSelf
                }
            }
        }
    }
