...
import com.point18.slg2d.world.common.getHeroBasicInfo
import java.util.*
...
class FightLogic(
    ...
    private fun loadForce(fightData: FightData, side: Int) {
        ...
        val heroEntityGroupList = hashMapOf<Int, LinkedList<Entity>>()
        for (heroData in fightData.heroList) {
            ...
            heroEntityGroupList.getOrPut(unitBaseProto.positionType) { LinkedList() }.add(heroEntity)

            this.initialHeroMap[heroEntity.id] = heroData
        }

        //设置站位
        for ((posType, entityList) in heroEntityGroupList) {
            entityList.sortBy { it.getFloatProperty(ARR_GONGJIJULI) }

            //不同排起始X坐标不同
            val lenX: Int
            val startX = when (posType) {
                FrontRow -> {
                    lenX = pcs.basicProtoCache.posTypeXSplit[2].second
                    pcs.basicProtoCache.posTypeXSplit[2].first
                }
                MiddleRow -> {
                    lenX = pcs.basicProtoCache.posTypeXSplit[1].second
                    pcs.basicProtoCache.posTypeXSplit[1].first
                }
                else -> {
                    lenX = pcs.basicProtoCache.posTypeXSplit[0].second
                    pcs.basicProtoCache.posTypeXSplit[0].first
                }
            }

            var index = 0
            for (entity in entityList) {
                val unitBaseProto = pcs.unitBaseCache.fetchProtoById(entity.getIntProperty(ProtoId))
                    ?: continue
                val offsetX = (unitBaseProto.basicPosition - 1) % lenX
                var offsetY = unitBaseProto.basicPosition / lenX
                if (offsetY + 1 > pcs.basicProtoCache.posTypeYSplit[index].second) {
                    offsetY = pcs.basicProtoCache.posTypeYSplit[index].second - 1
                }
                var posX = startX + offsetX
                val posY = pcs.basicProtoCache.posTypeYSplit[index].first + offsetY
                index++
                ...
                entity.setPos(posX.toDouble(), posY.toDouble())
            }
        }
    }
}
...
fun newEntityByHeroData(
    ...
    heroEntity.floatPropertyMap[Morale] =
        if (side == DEF_SIDE) {
            pcs.basicProtoCache.heroBattleDefaultMoraleWithEnemy
        } else {
            heroData.initMorale
        }
