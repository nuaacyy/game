...
class QuitAllianceEventHandler : IEventHandler<AreaCache> {
    ...
    override fun handleEvent(cache: AreaCache, event: Any, eventType: EventType, playerId: Long) {
        ...
        for (callBoss in allCallBoss) {
            ...
            rankInfo.memberRecordMap.remove(playerId)
            if (rankInfo.memberRecordMap.isEmpty()) {
                allianceRank.rank.removeValue(rankInfo)
                continue
            }
        ...
        for (activityBoss in allActivityBoss) {
            val personalRecord = activityBoss.atkRecordsMap[playerId]
            if (personalRecord == null) {
                continue
            }
        ...
        if (session != null) {
            ...
            inviteBuilders.forEach {
                newHunterInviteNotifier(Del, it).notice(session)
            }
        }
    }
}