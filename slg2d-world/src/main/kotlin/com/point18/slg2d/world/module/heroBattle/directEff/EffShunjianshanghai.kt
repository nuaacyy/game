...
class ShunjianshanghaiStrategy : DirectEffectStrategy {
    ...
    override fun doEffect(skillEff: SkillEffect, targetEntity: Entity) {
        ...
        val launchingSkill = launchEntity.launchingSkill
        ...
        if (skillEff.skillProto.type != UniqueSkill
            ...
            val hurt =
                calNormalHurt(launchEntity, targetEntity) * calBuffAddition(
                    launchEntity,
                    targetEntity
                )
            var skillHurt =
            ...
            if (skillEff.skillEffProto.rangeHarmRate != 0) {
                //计算距离影响
                val distance = calDistance(launchEntity, targetEntity).toInt()
                skillHurt = skillHurt * (10000 + skillEff.skillEffProto.rangeHarmRate * distance) / 10000
            }
            if (skillEff.skillEffProto.hpHarmRate != 0) {
                //计算敌方血量影响
                val hp = targetEntity.getIntProperty(Hp, false)
                val hpLimit = targetEntity.getIntProperty(ARR_HPLIMIT)
                val hpLoseRate = ((hpLimit - hp).toDouble() / hpLimit * 100).toInt()
                skillHurt = skillHurt * (10000 + skillEff.skillEffProto.hpHarmRate * hpLoseRate) / 10000
            }
            val calHurtCritRst = calHurtCrit(launchEntity, skillHurt)
        ...
        for ((_, buffList) in targetEntity.buffMap) {
            for (buff in buffList) {
                ...
                for (handle in handlerList) {
                    handle(launchEntity, buff, costHp)
                }
            }
        }
    }
