...
// PVE打BOSS战斗流程
class PveFightCommonBossAction(var action: Int) : IBossFightDeal {
    override fun dealAfterOneFight(
        ...
        if (monsterProto == null) {
            normalLog.error("PveFightCommonBossAction.kt  魔物的配置不存在:${commonBoss.bossId}")
            return false
        }

        val player = areaCache.playerCache.findPlayerById(atkFightData.playerId)
        if (player == null) {
            normalLog.error("PveFightCommonBossAction.kt  找不到玩家信息:${atkFightData.playerId}")
            return false
        }

        val personalRank = areaCache.commonBossCache.findPersonalCommonBossRankByXY(posX, posY)
        if (personalRank == null) {
            normalLog.error("PveFightCommonBossAction.kt  ($posX,$posY)对应的魔物的个人不存在")
            return false
        }
        ...
        var currentDamage = ((oldHp - currentHp) *
        ...
        commonBoss.nowHp = currentHp

        if (currentDamage > 0) {
            //更新个人排行
            val playerAtkRecord = commonBoss.atkRecordsMap.getOrPut(
                atkFightData.playerId
            ) { PersonalCommonBossAtkRecord(atkFightData.playerId, 0, getNowTime()) }
            ...
            if (player.allianceId != 0L) {
                ...
                if (allianceAtkRecord == null) {
                    allianceAtkRecord = AllianceCommonBossAtkRecord(player.allianceId, hashMapOf())
                }
            ...
            //更新个人攻击魔物记录
            areaCache.commonBossCache.updatePersonalAtkBoss(player.id, commonBoss)
            ...
            if (session != null) {
                val notifier = createHunterRecordNotifier(
                    ...
                    commonBoss.bossId,
                    commonBoss.nowHp,
                    commonBoss.calSameAllianceAtkRecordCount(areaCache, player.allianceId)
                )
                notifier.notice(session)
            }

            updateBossInvite(areaCache, commonBoss)
        ...
        //推送魔物受伤信息
        noticeMonsterDamageInfo(areaCache, posX, posY, commonBoss.bossId, currentDamage, atkGroup.id)

        //生成战报
        //========生成战斗奖励================
        if (commonBoss.nowHp <= 0) {
            // 击杀魔物
        ...
        if (lordProto != null) {
            reward.kingExp = (lordProto.lordExp *
                    getResearchEffPlusRate(
                        areaCache,
                        NO_FILTER,
                        player,
        ...
        // 打完魔物回家
        goHome(areaCache, posX, posY, atkGroup, true)
