...
import com.point18.slg2d.world.module.heroBattle.behaviorTree.node.ConditionNode
import com.point18.slg2d.world.module.heroBattle.behaviorTree.node.SelectorNode
import com.point18.slg2d.world.module.heroBattle.behaviorTree.node.SequenceNode
import com.point18.slg2d.world.module.heroBattle.fightLogic.actionNode.*
import com.point18.slg2d.world.module.heroBattle.fightLogic.conditionNode.*
import pb4client.*
...
class EntityManager(
    ...
    fun init() {
        for (entity in this.entityList) {
            ...
            // 统计攻防英雄数
            if (entity.getIntProperty(FightSide, false) == ATK_SIDE) {
                this.atkHeroNum++
            } else {
                this.defHeroNum++
            }
...
//            entity.update()
        ...
        if (this.cacheChangeProperty.isNotEmpty()) {
            this.cacheChangeProperty.forEach {
                val entity = this.entityMap[it.key] ?: return@forEach
                val intParaChange = it.value[0]
                val floatParaChange = it.value[1]
                var intParas: HashMap<Int, Int>? = null
                var floatParas: HashMap<Int, Double>? = null
                if (intParaChange != null) {
                    intParas = hashMapOf()
                    for (intType in intParaChange) {
                        intParas[intType] = entity.getIntProperty(intType)
                    }
                }
                if (floatParaChange != null) {
                    floatParas = hashMapOf()
                    for (floatType in floatParaChange) {
                        floatParas[floatType] = entity.getFloatProperty(floatType)
                    }
                }
                entity.manager.receiveLogRequest(entity.id, entity.id, LogProperty, intParas, floatParas)
            }
    ...
    fun receiveLogRequest(
        ...
        entityList: List<Entity>? = null,
        arrayParas: List<Pair<Int, Pair<HashMap<Int, Int>, HashMap<Int, Double>>>>? = null
    ) {
        this.recorder.receiveRecord(senderId, receiverId, behaviorType, intParas, floatParas, null)

//        println("第${this.currentTime / 100}帧,${this.entityMap[senderId]?.name}，行为:$behaviorType,intParas:$intParas,floatParas:$floatParas")
