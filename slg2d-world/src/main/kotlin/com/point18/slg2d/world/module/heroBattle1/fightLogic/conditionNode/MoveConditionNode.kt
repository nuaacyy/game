...
class MoveConditionNode : BNode<Entity>() {
    override fun execute(data: Entity): ActionResult {
        ...
        for (e in data.manager.entityList) {
            ...
            //过滤阵营
            if (camp == FriendlyForce && e.getIntProperty(FightSide, false) != data.getIntProperty(FightSide, false)) {
                continue
            }
            if (camp == EnemyForce && e.getIntProperty(FightSide, false) == data.getIntProperty(FightSide, false)) {
                continue
            }
            if (camp == SelfForce && e.intPropertyMap[AttachHeroId] != data.id) {
                continue
            }
