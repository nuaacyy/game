...
// 野外打怪
class WalkCommonBoss : BaseWalkStrategy(), IWalkDeal {
    init {
    ...
    override fun walkStartCheck(areaCache: AreaCache, wp: WalkParam, rs: IWalkDeal.WalkStartCheckResult?): Int {
        ...
        for (group in myGroups) {
            ...
            if (group.gotoRunType != WalkCommonBoss) {
                continue
            }
        ...
        // 目标格子判断
        val commonBoss = areaCache.commonBossCache.findCommonBossByXY(wp.gotoX, wp.gotoY)
        if (commonBoss == null) {
            return ResultCode.NO_FIGHT_BOSS.code
        }
        val bossId = commonBoss.bossId
