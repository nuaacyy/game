...
class ThreeKeyIndexSlice<K, T, U, V> {
    private var index = hashMapOf<K, HashMap<T, HashMap<U, LinkedList<V>>>>()
    ...
    ) : this(fetch1, fetch2, fetch3, equalFunc) {
    ...
    private fun move2NewIndex(key1: K, key2: T, key3: U, targetValue: V) {
        ...
        val valueList = valueMap2.getOrPut(key3) { LinkedList() }
    ...
    fun findByKey(key1: K, key2: T, key3: U, filter: (value: V) -> Boolean) {
        val valMap1 = this.index[key1]
        if (valMap1 === null) {
            return
        }
    ...
    fun findByKey(key1: K, key2: T, key3: U): LinkedList<V>? {
        val valMap1 = this.index[key1] ?: return null
        val valMap2 = valMap1[key2] ?: return null
        return valMap2[key3]
    }
    ...
    fun listMap(filter: (v: LinkedList<V>) -> Unit) {
        for ((_, valMap1) in this.index) {
            for ((_, valMap2) in valMap1) {
                for ((_, valList) in valMap2) {
                    filter(valList)
