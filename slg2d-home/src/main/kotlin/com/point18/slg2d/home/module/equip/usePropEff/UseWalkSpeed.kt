...
import pb4server.WalkSpeedAskReq
import java.util.*

class UseWalkSpeed : UseProp {
    ...
    override fun useProp(
        ...
        val speedRate = propProto.extend1.toIntOrNull()
        if (speedRate == null) {
            return UsePropReturn(ResultCode.PARAMETER_ERROR, "")
        }

        var rt = ResultCode.SUCCESS.code
        val askMsg = WalkSpeedAskReq.newBuilder()
        askMsg.groupId = groupId
        askMsg.speedRate = speedRate
        session.createACS<Home2WorldAskResp>().ask(
            session.worldShardProxy,
            session.fillHome2WorldAskMsgHeader { it.setWalkSpeedAskReq(askMsg) },
            ...
            try {
                when {
                    ...
                    askResp == null -> {
                        ...
                        if (askResp.walkSpeedAskRt.rt != ResultCode.SUCCESS.code) {
                            // todo 重试????? 由于W服的数据已经异常了,重试100次这个都是错的怎么办...
                            if (costs != null && costRes != null) {
                                helpers.resHelper.addResWithoutNotice(session, action, player, costs)
                            }
                            rt = askResp.walkSpeedAskRt.rt
                        } else {
                            if (costRes != null) {
                ...
                normalLog.error("WalkSpeedAskReq Error!", e)
                if (costs != null && costRes != null) {
                    helpers.resHelper.addResWithoutNotice(session, action, player, costs)
                }
