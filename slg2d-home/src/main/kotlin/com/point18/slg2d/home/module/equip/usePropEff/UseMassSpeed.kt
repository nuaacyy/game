...
import pb4server.MassSpeedAskReq
import java.util.*

class UseMassSpeed : UseProp {
            ...
            try {
                when {
                    ...
                    askResp == null -> {
                        ...
                        if (askResp.massSpeedAskRt.rt != ResultCode.SUCCESS.code) {
                            // todo 重试????? 由于W服的数据已经异常了,重试100次这个都是错的怎么办...
                            if (costs != null && costRes != null) {
                                helpers.resHelper.addResWithoutNotice(session, action, player, costs)
                            }
                            rt = askResp.massSpeedAskRt.rt
                        } else {
                            if (costRes != null) {
                ...
                normalLog.error("MassSpeedAskReq Error!", e)
                if (costs != null && costRes != null) {
                    helpers.resHelper.addResWithoutNotice(session, action, player, costs)
                }
