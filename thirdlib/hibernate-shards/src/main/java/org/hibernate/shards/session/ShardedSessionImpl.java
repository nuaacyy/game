...
public class ShardedSessionImpl implements ShardedSession, ShardedSessionImplementor, ShardIdResolver {
    ...
    private Map<ShardId, Shard> buildShardIdsToShardsMap() {
        final int virtualShardNumPerShard;
        if (shards.isEmpty()) {
            virtualShardNumPerShard = 0;
        } else {
            virtualShardNumPerShard = shards.get(0).getShardIds().size();
        }
        final int initialSize = (int) (shards.size() * virtualShardNumPerShard / 0.75 + 1.0);
        final Map<ShardId, Shard> map = new HashMap<ShardId, Shard>(initialSize);
    ...
    static List<Shard> buildShardListFromSessionFactoryShardIdMap(
        ...
        final List<Shard> shardList = Lists.newArrayListWithCapacity(sessionFactoryShardIdMap.size());
